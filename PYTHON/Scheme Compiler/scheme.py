...
def apply_primitive(procedure, args, env):
    ...
    return_list = list(args)
    if procedure.use_env:
        return_list.append(env)
    try:
        return procedure.fn(*return_list)
...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(vals) != len(formals):
            raise SchemeError("the number of variables and formals don't match")
        else:
            for count in range(len(vals)):
                frame.define(formals[count], vals[count])
...
def do_mu_form(vals):
    ...
    if len(vals) == 2:
        return MuProcedure(formals, vals.second[0])
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        "*** YOUR CODE HERE ***"
        function = vals[0].first
        if scheme_symbolp(function):
            env.define(function, scheme_eval(Pair("lambda", Pair(vals[0].second, vals.second)), env))
            return function
...
def do_let_form(vals, env):
    ...
    for item in bindings:
        if len(item) >= 3:
            raise SchemeError("wrong format")
        if not scheme_symbolp(item.first):
            raise SchemeError
        else:
            names = Pair(item.first, names)
            values = Pair(scheme_eval(item[1], env), values)
...
def do_or_form(vals, env):
    ...
    for i in range(len(vals)):
        eval_result = scheme_eval(vals[i], env)
        if scheme_true(eval_result):
            if scheme_symbolp(eval_result):
...
def check_formals(formals):
    ...
    for formal in formals:
        ...
        elif formal in used_formal:
            raise SchemeError("formal repeated")
        used_formal.append(formal)
