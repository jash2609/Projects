...
def extract_words(text):
    ...
    for letter in new_list:
        if letter in ascii_letters:
            ...
            word += " "
    return word.split()  # You may change/remove this line
...
def analyze_tweet_sentiment(tweet):
    ...
    total_sentiment = 0
    count_words_having_sentiment = 0
    for every_word in words:
        if has_sentiment(get_word_sentiment(every_word)):
            total_sentiment+=sentiment_value(get_word_sentiment(every_word))
            count_words_having_sentiment+=1
    if count_words_having_sentiment == 0:
        return make_sentiment(None)
    average_of_sentiments = make_sentiment(total_sentiment/count_words_having_sentiment)
    return average_of_sentiments
...
def find_centroid(polygon):
    ...
    while i < length: #formula from Wikipedia
        current = polygon[i]            
        next = polygon[i+1]
        common = (latitude(current) * longitude(next) - latitude(next) * longitude(current))  # The common term in the formula for area, lat and lon
...
def group_tweets_by_state(tweets):
    ...
    "*** YOUR CODE HERE ***"
    i=0 #counter
    couples = [] #list of pairs
    while i<len(tweets):
        location_of_tweet = tweet_location(tweets[i])
        keys=[] #list of keys
        distances=[] #list with nearest geo distances
        for key in us_states:
            keys += [key]
            group_polygons = us_states[key]
            distances+=[geo_distance(location_of_tweet, find_state_center(group_polygons))]
            min_distance_index = distances.index(min(distances)) #finds the index of the element, which has the minimum distance
        state = keys[min_distance_index] #returns the state, which is closest
        couples+=[[state, tweets[i]]]
        i+=1
    return group_by_key(couples)
...
def average_sentiments(tweets_by_state):
    ...
    """
    "*** YOUR CODE HERE ***"
    avg_state_sentiments={}
    for state_code in tweets_by_state:
        total_sentiments, sentiments_count = 0,0
        for tweet in tweets_by_state[state_code]:
            if has_sentiment(analyze_tweet_sentiment(tweet)) == True:
                total_sentiments+=sentiment_value(analyze_tweet_sentiment(tweet))
                sentiments_count+=1
        if sentiments_count!=0:
            avg_state_sentiments[state_code]=total_sentiments/sentiments_count
    return avg_state_sentiments
